...
/*void columnsGridInit() {
    ...
    for (int a=0;a<3;a++) {
        nextColumn[a] = rand() % 6 + 1;
    }
}
bool columnsValidMove(short x, short y) {
    if (y < 19 && columnsMap[x][y+3] == 0) {
        return true;
    } else {
        return false;
    }
...
void columnsLogic() {
    if (game.buttons.held(BTN_A,1)) {
        currentColumnDupe[0] = nextColumn[1];
        currentColumnDupe[1] = nextColumn[2];
        currentColumnDupe[2] = nextColumn[0];
        for (int a=0;a<3;a++) {
            nextColumn[a] = currentColumnDupe[a];
        }
    }
}
void columnsDraw() {
    ...
    for (int y=0;y<22;y++) {
        for (int x=0;x<9;x++) {
            switch (columnsMap[x][y]) {
            case 0 :
                break;
            case 1 :
                game.display.drawBitmap(x*4+36,y*4,red);
                break;
            case 2 :
                game.display.drawBitmap(x*4+36,y*4,yellow);
                break;
            case 3 :
                game.display.drawBitmap(x*4+36,y*4,green);
                break;
            case 4 :
                game.display.drawBitmap(x*4+36,y*4,cyan);
                break;
            case 5 :
                game.display.drawBitmap(x*4+36,y*4,blue);
                break;
            case 6 :
                game.display.drawBitmap(x*4+36,y*4,pink);
                break;
            }
        }
    }
    //draw rectangles
    game.display.color = 2;
    game.display.fillRectangle(0,0,36,88);
    game.display.fillRectangle(72,0,38,88);
    game.display.color = 0;
    game.display.fillRectangle(4,8,28,20);
    //draw next block
    for (int a=0;a<3;a++) {
        switch (nextColumn[a]) {
        case 1 :
            game.display.drawBitmap(16,12+(a*4),red);
            break;
        case 2 :
            game.display.drawBitmap(16,12+(a*4),yellow);
            break;
        case 3 :
            game.display.drawBitmap(16,12+(a*4),green);
            break;
        case 4 :
            game.display.drawBitmap(16,12+(a*4),cyan);
            break;
        case 5 :
            game.display.drawBitmap(16,12+(a*4),blue);
            break;
        case 6 :
            game.display.drawBitmap(16,12+(a*4),pink);
            break;
        }
    }
}*/
